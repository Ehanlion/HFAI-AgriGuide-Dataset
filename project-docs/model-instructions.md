...
- When generating outputs, do not reuse the same `explanation`, `confidence_statement`, `uncertainty_or_caution`, or `decision_support_notes` across rows. Evaluate each row independently and write row-specific content based on that row's crop, soil, weather, moisture, and nutrient values. Outputs may use a similar structure across rows, but copied or blanket text is not allowed. Consider this especially for the confidence, uncertainty, and decision columns where it is feasbile to copy content for simplicity sake, but is absolutely not allowed.
